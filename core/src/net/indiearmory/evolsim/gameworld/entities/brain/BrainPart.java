...
public abstract class BrainPart {
    ...
    private GameModel owner;

    private int nr_inputNodes, nr_hiddenNodes, nr_outputNodes;

    private float[] weights_input_hidden, weights_hidden_output;

    public BrainPart(GameModel owner, int nr_inputNodes, int nr_hiddenNodes, int nr_outputNodes){
        this.owner = owner;
    ...
    public BrainPart(GameModel owner, int nr_inputNodes, int nr_hiddenNodes, int nr_outputNodes, DNASequence dnaSequence){
        this.owner = owner;
        this.nr_inputNodes = nr_inputNodes;
        this.nr_hiddenNodes = nr_hiddenNodes;
        this.nr_outputNodes = nr_outputNodes;

        // Load up input->hidden weights from dna sequence
        weights_input_hidden = dnaSequence.getWeights(0);
        // Load up hidden->output weights from dna sequence
        weights_hidden_output = dnaSequence.getWeights(1);
    }
    ...
    public DNASequence getDNASequence(){
        ...
        dnaSequence.addWeights(weights_input_hidden);
        dnaSequence.addWeights(weights_hidden_output);
        return dnaSequence;
    }
